...
var handledSyscalls = []string{"mount"}
...
func LoadFilter() (libseccomp.ScmpFd, error) {
	...
	for _, sc := range handledSyscalls {
...
func Handle(fd libseccomp.ScmpFd, targetPID int, daemon daemonapi.InWorkspaceServiceClient) (stop chan<- struct{}, errchan <-chan error) {
	...
	go func() {
		for {
			...
			switch syscallName {
			// case "mount":
			// 	val, errno, cont = h.handleMount()
			// case "mkdir":
			// 	val, errno, cont = h.handleMkdir()
			default:
				val, errno, cont = handleUnknownSyscall(req)
			}
...
func handleUnknownSyscall(req *libseccomp.ScmpNotifReq) (val uint64, errno int32, cont bool) {
